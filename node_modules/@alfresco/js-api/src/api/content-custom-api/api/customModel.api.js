...
class CustomModelApi extends base_api_1.BaseApi {
    ...
    createCustomModel(status, description, name, namespaceUri, namespacePrefix, author) {
        assert_1.throwIfNotDefined(namespaceUri, 'namespaceUri');
        assert_1.throwIfNotDefined(namespacePrefix, 'namespacePrefix');
        const bodyParam = {
            status,
            description,
            name,
    ...
    createCustomType(modelName, name, parentName, title, description) {
        assert_1.throwIfNotDefined(modelName, 'modelName');
        assert_1.throwIfNotDefined(name, 'name');
        const bodyParam = {
            name,
            parentName,
            title,
    ...
    addPropertyToAspect(modelName, aspectName, properties) {
        assert_1.throwIfNotDefined(modelName, 'modelName');
        assert_1.throwIfNotDefined(aspectName, 'aspectName');
        const bodyParam = {
            'name': aspectName,
            properties
        };
    ...
    addPropertyToType(modelName, typeName, properties) {
        assert_1.throwIfNotDefined(modelName, 'modelName');
        assert_1.throwIfNotDefined(typeName, 'typeName');
        const bodyParam = {
            'name': typeName,
            properties
        };
    ...
    getCustomConstraints(modelName, constraintName, queryParams) {
        assert_1.throwIfNotDefined(modelName, 'modelName');
        assert_1.throwIfNotDefined(constraintName, 'constraintName');
        const pathParams = {
            modelName,
            constraintName
        };
    ...
    deleteCustomAspectProperty(modelName, aspectName, propertyName) {
        assert_1.throwIfNotDefined(modelName, 'modelName');
        assert_1.throwIfNotDefined(aspectName, 'aspectName');
        assert_1.throwIfNotDefined(propertyName, 'propertyName');
        const bodyParam = {
            'name': aspectName
        };
        const pathParams = {
